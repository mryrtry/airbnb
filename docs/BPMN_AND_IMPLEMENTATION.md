...
- Уведомление владельцу о публикации в BPMN (Activity_1r7wjdn, Activity_0qi5hke) в коде **не реализовано**: при смене статуса на PUBLISHED уведомление владельцу не отправляется. Владелец узнаёт об успешной публикации по ответу API или по своему списку листингов.
...
- Регистрация въезда: один вызов API переводит бронирование в CHECKED_IN и уведомляет владельца.  
  **Код:** `PUT /bookings/{id}/check-in`. Право: `BOOKING_READ` или `ALL_BOOKING_UPDATE`.  
  `BookingController.recordCheckIn()` → `BookingServiceImpl.recordCheckIn()`: `BookingAccessService.requireCanUpdate(id)` (доступ только у гостя этого бронирования, владельца листинга или админа); проверка статуса APPROVED; установка CHECKED_IN и `checkInAt`, сохранение; `notifyUser(listing.getOwnerId(), BOOKING_CHECKED_IN, Map.of("bookingId", id))`.  
  Файлы: `BookingController`, `BookingServiceImpl`, `BookingAccessServiceImpl.requireCanUpdate`.
...
- Регистрация выезда: перевод в CHECKED_OUT, уведомление владельца и **автоматическое открытие окна резолюции**.  
  **Код:** `PUT /bookings/{id}/check-out`. Право: те же, что для check-in.  
  `BookingServiceImpl.recordCheckOut()`: `requireCanUpdate(id)`, проверка статуса CHECKED_IN, установка CHECKED_OUT и `checkOutAt`, сохранение, `EntityEvent(UPDATED, dto)`, `notifyUser(..., BOOKING_CHECKED_OUT, ...)`, затем `eventPublisher.publishEvent(new BookingCheckedOutEvent(booking.getId()))`.  
...
- Шлюз «Существенное происшествие?» (Gateway_0guf1pf): в BPMN после Sys_HandleEscalation система ветвится на «Да» (назначить гостю обязательную выплату, уведомить гостя) и «Нет» (уведомить владельца об урегулировании конфликта). В коде это **не реализовано**: после эскалации только смена статуса на ESCALATED и уведомление гостю. Нет ни автоматического ветвления, ни роли «поддержка», ни API для «назначить обязательную выплату» или «урегулировать без взыскания». При необходимости это можно добавить отдельным сервисом/эндпоинтами.

- Назначить гостю обязательную выплату (Activity_0mozg5p), уведомить гостя об обязательной выплате (Activity_1t1yyy6), сценарий «гость обязан оплатить» в коде **не реализованы**: нет статуса «обязательная выплата» и принудительного сценария, только добровольные respond-pay/respond-refuse.

- Уведомить владельца об урегулировании конфликта (Activity_1w8qgok) при ответе «Нет» на существенное происшествие в коде **не реализовано**.

Итог: эскалация владельцем и уведомление гостя об эскалации реализованы; ветвление по «существенное происшествие?», обязательная выплата и уведомление владельцу об урегулировании без взыскания — нет.
...
- Таймер в BPMN — промежуточное событие по времени (14 дней). В коде таймер реализован задачей по расписанию: раз в час (по умолчанию) вызывается метод, который закрывает просроченные окна.  
  **Код:** `ResolutionScheduler.closeExpiredResolutionWindows()` (аннотация `@Scheduled(cron = "${airbnb.resolution.scheduler.cron:0 0 * * * *}")`) вызывает `ResolutionService.closeExpiredWindows()`.  
  В `ResolutionServiceImpl.closeExpiredWindows()`: выбираются окна со статусом OPEN и `openedAt` старше `ResolutionConstants.WINDOW_DAYS` (14 дней); для каждого устанавливается CLOSED и `closedAt`, сохранение; `notifyUser(listing.getOwnerId(), RESOLUTION_WINDOW_CLOSED_AUTO, Map.of("bookingId", ..., "resolutionId", ...))`.  
  Файлы: `resolution/config/ResolutionScheduler.java`, `resolution/constants/ResolutionConstants.java` (WINDOW_DAYS = 14), `resolution/service/ResolutionServiceImpl.closeExpiredWindows()`.

- Окна в статусах MONEY_REQUESTED или REFUSED по таймеру не закрываются — только OPEN. Если в BPMN подразумевается закрытие и других состояний по истечении срока, в коде это нужно добавить отдельно.

- Ручное закрытие окна (админ/система):  
  **Код:** `PUT /resolutions/{id}/close`. Право: только `ALL_RESOLUTION_UPDATE`.  
  `ResolutionServiceImpl.close()`: `ResolutionAccessService.requireCanClose(id)` (разрешено только при наличии ALL_RESOLUTION_UPDATE); установка CLOSED и `closedAt`, сохранение; `notifyUser(..., RESOLUTION_WINDOW_CLOSED, ...)`.

Итог: автоматическое закрытие через 14 дней и уведомление владельца о закрытии реализованы; ручное закрытие — только для роли с ALL_RESOLUTION_UPDATE.
...
- **Бронирования:** `BookingAccessService` — `requireCanRead(bookingId)`, `requireCanUpdate(bookingId)`, `requireCanDecide(bookingId)`, `getScopeUserIdForList()`. Список бронирований (GET /bookings) для не-админа ограничивается теми, где пользователь гость или владелец листинга (`BookingSpecifications.scopeForUser`, `findWithFilter(..., scopeUserId)`).  
...
## 6. Что не покрыто или упрощено по BPMN

- **Шлюз «Существенное происшествие?» (Gateway_0guf1pf):** ветвление после эскалации не реализовано. Нет ни автоматического решения, ни API для «поддержки» выбора «да/нет» и последующих шагов.

- **Назначить гостю обязательную выплату (Activity_0mozg5p), уведомить гостя об обязательной выплате (Activity_1t1yyy6):** нет статуса обязательной выплаты и принудительного сценария оплаты; только добровольные respond-pay/respond-refuse.

- **Уведомить владельца об урегулировании конфликта (Activity_1w8qgok)** при ответе «Нет» на существенное происшествие: не реализовано.

- **Явное «Есть смысл продолжать?» (Gw_GuestRefused) при отказе гостя:** владелец либо вызывает эскалацию, либо нет; отдельного «закрыть без эскалации» из REFUSED в коде нет.

- **Уведомление владельцу о публикации объявления (Activity_1r7wjdn, Activity_0qi5hke):** при смене статуса листинга на PUBLISHED уведомление не отправляется.

- **Таймер 14 дней:** закрываются только окна в статусе OPEN; окна в MONEY_REQUESTED или REFUSED по истечении 14 дней не закрываются автоматически (при необходимости это можно уточнить в BPMN и доработать).
...
| Уведомить владельца о публикации | Не реализовано | — |
...
| Существенное происшествие? / обязательная выплата / уведомить об урегулировании | Не реализовано | — |
| Не отвечает 14 дней → закрыть окно → уведомить владельца | ResolutionScheduler → closeExpiredWindows() → RESOLUTION_WINDOW_CLOSED_AUTO | ResolutionScheduler, ResolutionServiceImpl.closeExpiredWindows |
| Закрыть окно вручную | close() → RESOLUTION_WINDOW_CLOSED | PUT /resolutions/{id}/close (ALL_RESOLUTION_UPDATE) |
